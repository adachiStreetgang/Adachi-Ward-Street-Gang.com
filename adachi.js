    ...
    // Old spray-paint loader removed; replaced by new simple `#siteLoader` implementation.
        ...
        // Screenshots carousel constructor
        function createCarousel(options) {
            const { slidesId, dotsId, containerClass, autoplayMs = 5000 } = options;
            
            const slidesEl = document.getElementById(slidesId);
            if(!slidesEl) return;
            
            const container = containerClass ? document.querySelector(containerClass) : slidesEl.closest('.screenshots-container');
            if(!container) return;
            ...
            return { next, prev, go, update, startAutoplay, pauseAutoplay };
        }
        ...
        (function(){
            ...
            createCarousel({
                slidesId: 'ssSlides',
                dotsId: 'ssDots',
                containerClass: '.screenshots-container:not(.vrc-screenshots)',
                autoplayMs: 5000
            });

            // VRC screenshots carousel
            createCarousel({
                slidesId: 'vrcSlides',
                dotsId: 'vrcDots',
                containerClass: '.vrc-screenshots',
                autoplayMs: 6000 // slightly different timing
            });
        })();
